...
export class SidebarDrawerComponent implements OnInit, OnDestroy {
    ...
    smoothingFactorFormControl = new FormControl(
        {
            value: ToolDefaults.defaultSmoothingSettings.factor,
            disabled: !ToolDefaults.defaultSmoothingSettings.isEnabled,
        },
        [
            Validators.required,
            Validators.pattern(Regexes.integerRegex),
    ...
    junctionDiameterFormControl = new FormControl(
        {
            value: ToolDefaults.defaultJunctionSettings.diameter,
            disabled: !ToolDefaults.defaultJunctionSettings.isEnabled,
        },
        [
            Validators.required,
            Validators.pattern(Regexes.integerRegex),
    ...
    zoomFormControl = new FormControl(ZoomInfo.defaultZoomPercentage, [
        Validators.required,
        Validators.pattern(Regexes.integerRegex),
        Validators.min(ZoomInfo.minimumZoomPercentage),
        Validators.max(ZoomInfo.maximumZoomPercentage),
    ]);
    ...
    ngOnInit(): void {
        ...
        this.smoothingEnabledChangedSubscription = this.smoothingEnabledFormControl.valueChanges.subscribe(() => {
            ...
            this.smoothingEnabledFormControl.value
                ? this.smoothingFactorFormControl.enable({
                      emitEvent: false,
                  })
                : this.smoothingFactorFormControl.disable({
                      emitEvent: false,
                  });
        });
        ...
        this.zoomPercentChangedSubscription = this.zoomFormControl.valueChanges.subscribe(() => {
            if (this.zoomFormControl.valid && this.drawingService.zoomPercentage !== this.zoomFormControl.value) {
                this.drawingService.zoomPercentage = this.zoomFormControl.value;
            }
        });
        this.zoomFieldChangedSubscription = this.drawingService.zoomPercentChangedSubscription$.subscribe(() => {
            this.zoomFormControl.setValue(this.drawingService.zoomPercentage);
        });
        ...
        this.resetCanvasViewShortcutSubscription = this.shortcutService.resetZoomShortcut$.subscribe(() => {
